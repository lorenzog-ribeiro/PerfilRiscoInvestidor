...
const TesteA: React.FC<{ perfilData: PerfilData }> = ({ perfilData }) => {
    const [valor, setValor] = useState<number>(Math.random() * 2); // Valor aleatório entre 0 e 2
    const [posicao, setPosicao] = useState<{ cx: number; cy: number }>({ cx: 200, cy: 125 });
    const [linhaFinal, setLinhaFinal] = useState<{ x: number; y: number }>({ x: 220, cy: 160 });
    ...
    // Função para alterar o valor ao clicar no botão (para teste da animação)
    const mudarValor = () => {
        setValor(Math.random() * 2);
    };
            ...
            <CardContent className="space-y-8">
                <div className="flex justify-center">
                    ...
                    <svg viewBox="0 0 400 250" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <line x1="50" y1="200" x2="350" y2="50" stroke="blue" stroke-width="2" stroke-dasharray="6,4" />
                        ...
                        {/* Bolinha laranja (teleférico) - reposicionada no final da linha cinza */}
                        <circle
                            cx={linhaFinal.x}
                            cy={linhaFinal.y}
                            r="5"
